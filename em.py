...
def generate_population(mu, N=1000, max_sigma=0.5, mean_sigma=0.08):
    ...
    sigma = 1 / np.sqrt(expon.rvs(loc=exp_min_size,
                                  scale=exp_mean_size, size=N))
...
N = 60
a = 0.45
m1 = 0  # true mean 1 this is what we want to guess
m2 = 0.4  # true mean 2 this is what we want to guess
...
py.hist(s, bins=np.r_[-1:2:0.025], alpha=0.3,
        color='g', histtype='stepfilled')
ax = py.twinx()
ax.grid(False)
...
while (improvement > delta):
    ...
    for k in [0, 1]:
        # responsibility
        gamma[k, :] = pi_[k] * \
        py.normpdf(s, mu[k], sig[k]) / pdf_model(s, p_new)
        # effective number of objects to k category
        N_[k] = 1. * gamma[k].sum()
        # new sample mean of k category
        mu[k] = sum(gamma[k] * s) / N_[k]
        # new sample var of k category
